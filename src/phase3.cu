...
static void chk3(const char* n, bool ok) {
    if (ok) { printf("  [PASS] %s\n", n); p3_pass++; }
    else { printf("  [FAIL] %s\n", n); p3_fail++; }
}
...
int TinyModel::logits_to_token(MemPool& scratch, Tensor& hidden, const std::vector<int>& past_ids, GenerationConfig config) {
...
int TinyModel::decode_next(MemPool& scratch, int token_id, int pos, const std::vector<int>& past_ids, GenerationConfig config) {
    scratch.reset();
    IntTensor tid(scratch, 1);
    ...
    return logits_to_token(scratch, x, past_ids, config);
}
...
int TinyModel::generate_next_full(MemPool& scratch, const std::vector<int>& ids, GenerationConfig config) {
    ...
    if (config.repetition_penalty != 1.0f && !ids.empty()) {
        ...
        k_apply_repetition_penalty(last.data, d_past.data, ids.size(), config.repetition_penalty);
    }
...
std::vector<int> TinyModel::generate_full(MemPool& scratch, const std::vector<int>& prompt, GenerationConfig config) {
    ...
    for (int t = 0; t < config.max_new_tokens; t++) {
        ...
        ids.push_back(nxt);
        if (nxt == config.eos_token_id) break;
...
std::vector<int> TinyModel::generate_cached(MemPool& scratch, const std::vector<int>& prompt, GenerationConfig config) {
    ...
    int nxt = logits_to_token(scratch, keep, ids, config);
    ids.push_back(nxt);
    ...
    for (int t = 1; t < config.max_new_tokens; t++) {
        ...
        nxt = decode_next(scratch, ids[pos], pos, ids, config);
        ids.push_back(nxt);
