...
void LlamaTokenizer::load(const char* path) { 
    ...
    for (int i = 0; i < vs; i++) { 
        ...
        if (l > 0 && l < 1000) {
            std::string s(l, '\0'); fread(&s[0], 1, l, f); 
            vocab[i] = s; 
        }
    } 
    fclose(f); 
...
// --- MODEL CORE ---
LlamaLayerMixed::LlamaLayerMixed(MemPool& pool, int seq, int d, int hd, int nh, int nkv) : 
...
void LlamaLayerMixed::forward_decode_graph(MemPool& scratch, HalfTensor& x, HalfTensor& out, int* d_pos) {
    ...
    k_half_copy_to_cache_graph(K.data, k_cache.data, d_pos, n_kv_heads * hd); 
    k_half_copy_to_cache_graph(V.data, v_cache.data, d_pos, n_kv_heads * hd);
...
Llama2MixedGraph::Llama2MixedGraph(MemPool& pool) : tok_embed(pool, {vocab, dim}), norm_w(pool, {dim}), lm_head(pool, {vocab, dim}) {
    cublasCreate(&handle); 
    d_pos = pool.alloc<int>(1); d_token = pool.alloc<int>(1); d_out = pool.alloc<int>(1);
...
void Llama2MixedGraph::capture_graph(MemPool& scratch) {
    ...
    // Convert to FP32 for CPU processing
    ...
    if(err != cudaSuccess) { graph_exec = nullptr; }
}

void Llama2MixedGraph::chat(MemPool& scratch, const std::vector<int>& prompt_ids, GenerationConfig cfg) {
    printf("\n[DEBUG] Running Prefill Phase...\n"); fflush(stdout);
    
    std::vector<int> prefill_ids(prompt_ids.begin(), prompt_ids.end() - 1);
    if (prefill_ids.size() > 0) prefill(scratch, prefill_ids); 
    
    printf("[DEBUG] Compiling CUDA Graph... "); fflush(stdout);
    capture_graph(scratch);
    
    printf("Done!\n\n[TinyLlama INT8 + CUDA Graphs]: "); fflush(stdout);
    
    int pos = prefill_ids.size();
    std::vector<int> past;
    for(int id : prompt_ids) { 
        printf("%s", tokenizer.decode(id).c_str()); fflush(stdout); 
        past.push_back(id); 
    }
    
    auto t1 = std::chrono::high_resolution_clock::now();
    int token = prompt_ids.back();
    int tokens_generated = 0;
    
    // CPU buffer for logits (to apply Repetition Penalty natively)
    std::vector<float> h_logits(vocab);

    for (int i=0; i<cfg.max_new_tokens; i++) {
        // SYNCHRONOUS COPIES to guarantee the Graph reads the exact updated values!
        cudaMemcpy(d_pos, &pos, sizeof(int), cudaMemcpyHostToDevice); 
        cudaMemcpy(d_token, &token, sizeof(int), cudaMemcpyHostToDevice);
        
        // Execute the neural network layer stack instantly
        cudaGraphLaunch(graph_exec, 0);
        
        // The graph output is sitting in the scratch pool at the end. Let's grab it!
        // We know `logits32.data` is the last float array allocated in `scratch`.
        ...
        // CPU-SIDE REPETITION PENALTY & ARGMAX
        for (int p_id : past) {
            float val = h_logits[p_id];
            h_logits[p_id] = val > 0 ? val / 1.1f : val * 1.1f; // Standard 1.1x penalty
        }
        
        int best_id = 0;
        float best_val = -1e9f;
        for (int v = 0; v < vocab; v++) {
            if (h_logits[v] > best_val) { best_val = h_logits[v]; best_id = v; }
        }
        token = best_id;
        
        if (token == 2 || token == 0) break; 
