...
__global__ void _mha_scores_fused_mask(float* Q, float* K, float* S, int seq, int n_heads, int head_dim) {
    ...
    if (h < n_heads && r < seq && c < seq) {
        ...
        float sum = 0.0f;
        int q_base = r * (n_heads * head_dim) + h * head_dim, k_base = c * (n_heads * head_dim) + h * head_dim;
...
__global__ void _mha_weighted_sum(float* P, float* V, float* O, int seq, int n_heads, int head_dim) {
    ...
    if (h < n_heads && r < seq && d < head_dim) {
        float sum = 0.0f;
        for (int c = 0; c < seq; c++) sum += P[h * seq * seq + r * seq + c] * V[c * (n_heads * head_dim) + h * head_dim + d];
...
void k_mha_scores_fused_mask(float* Q, float* K, float* S, int seq, int n_heads, int head_dim) { dim3 g((seq + 31) / 32, seq, n_heads), b(32); _mha_scores_fused_mask<<<g, b>>>(Q, K, S, seq, n_heads, head_dim); }
...
void k_mha_weighted_sum(float* P, float* V, float* O, int seq, int n_heads, int head_dim) { dim3 g((head_dim + 31) / 32, seq, n_heads), b(32); _mha_weighted_sum<<<g, b>>>(P, V, O, seq, n_heads, head_dim); }
